...
class AnalysisAndForm:
    ...
    def _init_widget(self):
        ...
        self._build_OR_text(height=1, width=2, row=1, column=3, columnspan=2, pady=(10,0))
        self._build_original_button()
        self._build_reflection_button()
        self._build_OR_text(height=1, width=2, row=2, column=3, columnspan=2, pady=(5,0))
    ...
    def _build_normalisation_button(self):
        self.normalisation_button = make_button(self.root, text='Normalisation', command=self.__normal, row=1, column=0, outer_pady=0, 
            outer_padx=(20,0), columnspan=3)

    def _build_original_button(self):
        self.original_button = make_button(self.root, text='Original', 
            command=self.__original, row=1, column=5, outer_pady=0, 
            outer_padx=(0,20), columnspan=3)

    def _build_reflection_button(self):
        self.reflection_button = make_button(self.root, text='Reflection', command=self.__reflect, row=2, column=0, outer_pady=5, 
            outer_padx=(20,0), columnspan=3)

    def _build_absorbance_button(self):
        self.absorbance_button = make_button(self.root, text='Absorbance', 
            command=self.__absorb,row=2, column=5, outer_pady=5, 
            outer_padx=(0,20), columnspan=3)
    ...
    def _build_idx_title(self):
        self.idx = make_text(self.root, content="Individual Index: ", 
            bg=tkcolour_from_rgb(PASTEL_ORANGE_RGB), column=0, row=4, width=18, columnspan=8, pady=(10, 5))

    def _build_idxs(self):
        self.idx1 = make_button(self.root, text='1', command=lambda:self.__idxn(1), row=5, column=0, outer_pady=5, outer_padx=(10,5), width=1)
        ...
        self.idx8 = make_button(self.root, text='8', command=lambda:self.__idxn(8), row=5, column=7, outer_pady=5, outer_padx=(5, 10), width=1)
        return

    def _build_stO2_text(self):
        self.stO2_text = make_text(self.root, content="Saturation index: ", 
            bg=tkcolour_from_rgb(PASTEL_ORANGE_RGB), column=0, row=7, width=18, columnspan=4, pady=(10, 0))

    def _build_stO2(self):
        self.stO2 = make_entry(self.root, row=7, column=4, width=15, pady=(10,0), padx=(0, 20), columnspan=4)

    def _build_perf_text(self):
        self.perf_text = make_text(self.root, content="Perfusion index: ", 
            bg=tkcolour_from_rgb(PASTEL_ORANGE_RGB), column=0, row=8, width=17, columnspan=4, pady=(5,0))

    def _build_perf(self):
        self.perf = make_entry(self.root, row=8, column=4, width=15, pady=(5,0), padx=(0, 20), columnspan=4)


    def _build_hemo_text(self):
        self.hemo_text = make_text(self.root, content="Hemoglobin index: ", 
            bg=tkcolour_from_rgb(PASTEL_ORANGE_RGB), column=0, row=9, width=18, columnspan=4, pady=(5,0))

    def _build_hemo(self):
        self.hemo = make_entry(self.root, row=9, column=4, width=15, pady=(5,0), padx=(0, 20), columnspan=4)

    def _build_tissue_text(self):
        self.stO2_text = make_text(self.root, content="Tissue water index: ", 
            bg=tkcolour_from_rgb(PASTEL_ORANGE_RGB), column=0, row=10, width=20, columnspan=4, pady=(5, 15))

    def _build_tissue(self):
        self.stO2 = make_entry(self.root, row=10, column=4, width=15, pady=(5, 15), padx=(0, 20), columnspan=4)
