...
class NewColour:
    ...
    def _build_wl(self):
        self.wl_button = make_button(self.root, text='WL', width=3, command=self.__update_to_wl, row=1, column=0, inner_pady=5, outer_padx=(20, 0), columnspan=1)
        self.wl_checkbox = make_checkbox(self.root, "", row=1, column=0, var=self.wl_checkbox_value, sticky=NE, inner_padx=0, inner_pady=0, outer_padx=0)
        self.wl_checkbox.deselect()
        self.wl_checkbox.bind('<Button-1>', self.__update_wl_check_status)

    def _build_idx(self):
        self.idx_button = make_button(self.root, text="IDX", width=3, row=1, column=1, command=self.__update_to_idx, inner_pady=5, columnspan=1, outer_padx=0)
        self.idx_checkbox = make_checkbox(self.root, "", row=1, column=1, var=self.idx_checkbox_value, sticky=NE, inner_padx=0, inner_pady=0, outer_padx=(0, 25))
        self.idx_checkbox.deselect()
        self.idx_checkbox.bind('<Button-1>', self.__update_idx_check_status)

    def _build_gs(self):
        self.gs_button = make_button(self.root, text='GS', width=3, command=self.__update_to_gs, row=1, column=2, columnspan=1, inner_pady=5, outer_padx=(0, 20))
        self.gs_checkbox = make_checkbox(self.root, "", row=1, column=2, var=self.gs_checkbox_value, sticky=NE, inner_padx=0, inner_pady=0, outer_padx=(0, 20))
    ...
    def _build_save_wo_scale(self):
        self.save_wo_scale_label = make_label(self.root, "Save W/O Scale", row=8, column=1, columnspan=2, outer_padx=(0, 15), outer_pady=(10, 15), inner_padx=10, inner_pady=5)
        self.save_wo_scale_checkbox = make_checkbox(self.root, text="", row=8, column=2, var=self.save_wo_scale_checkbox_value, sticky=NE, inner_padx=0, inner_pady=0, outer_pady=(10, 0), outer_padx=(0, 37))
    ...
    def _build_norm_og(self):
        self.norm_button = make_button(self.root, text="NORM", row=6, column=2, columnspan=1, command=self.__norm, inner_padx=3, inner_pady=0, outer_padx=(20, 15), outer_pady=5, width=5)
        self.og_button = make_button(self.root, text="OG", row=7, column=2, columnspan=1, command=self.__og, inner_padx=3, inner_pady=0, outer_padx=(20, 15), outer_pady=(5, 0), width=5)

    def _build_new_image(self):
        if self.new_colour_image_data is None:
            # Placeholder
            self.new_colour_image = make_label(self.root, "new_colour image placeholder",row=2, column=0, rowspan=4, columnspan=3, inner_pady=50, inner_padx=50, outer_padx=15, outer_pady=(15, 10))
        else:
            logging.debug("BUILDING NEW COLOUR IMAGE...")
            (self.new_colour_image_graph, self.new_colour_image, self.image_array) = make_image(self.root, self.new_colour_image_data,row=2, column=0, columnspan=3, rowspan=4, lower_scale_value=self.lower_scale_value, upper_scale_value=self.upper_scale_value,color_rgb=BACKGROUND, gs=self.gs)
