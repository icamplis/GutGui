...
def init():
    '''
    Creates a root window which contains a notebook 'notebook' with three frames: 'introduction', 'input_output', and 'image_diagram'. Returns the root window along with the three frames. 
    '''
    root = Tk()
...
def make_label_button(window, text, command, width):
    ...
    based on the input parameters. The button is given solid relief, coloured white, and given a 2px thick black border. The button is padded 15px from the left and 15px above and below.
    window : tk.Frame
    command : function
...
def make_text(window, content, row, column, padx=0, pady=0, height=1, width=2,
    ...
    columspan : int (default 1)
    rowspan : int (default 1)
    state : DISABLED or NORMAL (default DISABLED, NORMAL allows for the text to be selected/copied/edited while DISABLED does not)
    '''
...
def make_entry(window, row, column, width, columnspan=1, pady=0, 
    ...
    '''
    Creates an entrybox, grids it, and returns it based on the input parameters.
    window : tk.Frame
    row : int
    column : int
    width : int (text units)
...
def make_info(title, info):
    '''
    Creates a pop-up TopLevel() window in the top=left screen corner containing information about the widget. Default size is 50 characters wide by 20 characters high, with wrapping by word, disabled text, 5px padding around the text box, and resizable=False. 
...
def find_nearest(array, value):
    ...
    # take the absolute value of the difference between each value in the input array and the input value, and return the index of the minimum value
    return (np.abs(array - value)).argmin()

def rgb_to_angle(rgb):
    '''
    Converts an rgb tuple or list to an angle and returns the angle. Convertion is done by flattening the rgb colour space along axis 2 and determining the angle of the flattened rgb point (red). This process is repeated with axis 0 as the flattened axis (blue). If red is 0, we return blue as the angle. Similarly if blue is 315, we return red as the angle. Otherwise, we return the average of the two angles.
...
def angle_to_jet(angle):
    '''
    Returns the index of the jet angle array nearest to the input angle, giving the index of the jet colour scale that most closely matches the rgb value that formed the input angle.
    angle : int or float
    '''
...
def rgb_image_to_jet_array(img_array):
    ...
    for i in range(len(img_array)):
        ...
        for j in range(len(img_array[i])):
            ...
            # convert rgb to jet value using rgb_to_angle and angle_to_jet and append to the array to form the 2D matrix or jet values
