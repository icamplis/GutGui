...
class OGColourData:
    ...
    def _build_data(self):
        # mean
        self.mean_text = make_text(self.root, content="Mean = " + str(self.mean_value), bg=tkcolour_from_rgb(PASTEL_PINK_RGB), column=0, row=1, width=13, columnspan=1, padx=(15, 15), state=NORMAL)
        # standard deviation
        self.sd_text = make_text(self.root, content="SD = " + str(self.sd_value), bg=tkcolour_from_rgb(PASTEL_PINK_RGB), column=0, row=2, width=11, columnspan=1, padx=0, state=NORMAL)
        # median
        self.median_text = make_text(self.root, content="Median = " + str(self.median_value), bg=tkcolour_from_rgb(PASTEL_PINK_RGB), column=0, row=3, width=15, columnspan=1, padx=(15, 15), state=NORMAL)
        # IQR
        self.iqr_text = make_text(self.root, content="IQR = " + str(self.iqr_value), bg=tkcolour_from_rgb(PASTEL_PINK_RGB), column=0, row=4, width=6+len(str(self.iqr_value)), columnspan=1, padx=(15, 15), state=NORMAL)
        # min
